...
export const getProductWithSizesAndColors = async (productId) => {
  ...
  return await axios.get(process.env.REACT_APP_API+`/pro/getProducts`);
}
...
export const getProductWithVarients = async (product) => {
  ...
  return await axios.post(process.env.REACT_APP_API+`/pro/getProductWithVarients`, product);
}
...
export const getBlueShirts = async () => {
  // return await axios.get(`http://localhost:8000/api/products/${count}`);
  return await axios.get(process.env.REACT_APP_API+`/blue-shirts`);
}
